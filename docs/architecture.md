...
- Las **APIs** vuelven a validar sesión en servidor (`requireTenantSession` para comercio, `requireSuperAdminSession` para `/api/admin`) y derivan `tenantId` del token, no del body.
