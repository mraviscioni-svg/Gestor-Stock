...
SaaS **multi-tenant** para **catálogo, ventas y stock**: lector de código de barras tipo teclado (HID), ajustes de inventario y panel operativo. Pensado para **Vercel Hobby** + **PostgreSQL** externo (**Neon** o **Supabase**) y **Prisma ORM**. El panel del comercio usa rutas como **`/dashboard`** (el tenant lo define la sesión); el **admin de plataforma** (`SUPER_ADMIN`) en **`/admin`**.
...
Definidas en código en [`src/config/demo-auth-defaults.ts`](src/config/demo-auth-defaults.ts). Tras `npm run db:seed` o `POST /api/internal/bootstrap`, el login usa **usuario + contraseña** (p. ej. `dueno` / `admin`); el email en la DB es opcional y solo para contacto.
