...
import { PrismaClient, PaymentMethod, Role, StockMovementReason } from "@prisma/client";
...
async function main() {
  ...
  if (!demoPassword || demoPassword.length < 8) {
    throw new Error(
      "SEED_DEMO_PASSWORD must be set in the environment and be at least 8 characters before running seed."
    );
  }
  ...
  console.log("Seed OK:", { tenantId: tenant.id, ownerEmail: owner.email });
}
